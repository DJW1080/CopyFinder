...
```text
CopyFinder-v2.0.9-win-x64-Standalone.zip
CopyFinder-v2.0.9-win-x64-Standalone.zip.sha256.txt
```
...
```powershell
$zip = ".\CopyFinder-v2.0.9-win-x64-Standalone.zip"
$expected = (Get-Content ".\CopyFinder-v2.0.9-win-x64-Standalone.zip.sha256.txt").Split(" ")[0]
...
```powershell
Expand-Archive -LiteralPath ".\CopyFinder-v2.0.9-win-x64-Standalone.zip" -DestinationPath "C:\Tools\CopyFinder" -Force
```
